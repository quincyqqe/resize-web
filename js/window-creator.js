...
export class WindowCreator {
  ...
  handleWindowDrop(targetWindowEl, inputNumber) {
    ...
    this.inputs[oldInput].windows = this.inputs[oldInput].windows.filter(
      (win) => {
        if (win.id === windowId) {
  ...
  expandWindowToFitOutput(windowEl, outputEl) {
  const canvasRect = this.canvas.getBoundingClientRect();
  const outputRect = outputEl.getBoundingClientRect();
  ...
  createWindow(x, y, width, height) {
    ...
    windowEl.addEventListener("dblclick", () => {
      const windowRect = windowEl.getBoundingClientRect();
      const canvasRect = this.canvas.getBoundingClientRect();

      const x = windowRect.left - canvasRect.left;
      const y = windowRect.top - canvasRect.top;

      const targetOutput = getOutputAtPosition(x, y, this.canvas);
      if (targetOutput) {
        this.expandWindowToFitOutput(windowEl, targetOutput);
      }
    });
    ...
    const windowInfo = {
      ...
      virtualSize: calculateVirtualSize(
        width,
        height,
        this.canvas,
        this.totalVirtualWidth,
  ...
  closeWindow(windowId) {
    ...
    if (windowEl) {
      ...
      if (windowInfo) {
        ...
        this.inputs[windowInfo.input].windows = this.inputs[
          windowInfo.input
  ...
  updateGlobalWindowNumbers() {
    this.windows.forEach((windowInfo, index) => {
      ...
      const windowEl = document.getElementById(windowInfo.id);
      if (windowEl) {
        windowEl.querySelector(".resize-handle").textContent =
          windowInfo.number;
      }
    });
  }
